...
USER root
RUN chown default -R /src/github.com/grafana/grafana &&\
    ln -s /opt/rh/go-toolset-1.12/root/usr/bin/go /usr/bin/go
USER default
...
RUN yarn install --pure-lockfile --no-progress --network-timeout 1000000
...
RUN mkdir -p "$GF_PATHS_HOME/.aws" && \
    groupadd -g $GF_GID grafana && \
    adduser -u $GF_UID -g grafana grafana && \
...
COPY --from=0 src/github.com/grafana/grafana/bin/linux-amd64/grafana-server src/github.com/grafana/grafana/bin/linux-amd64/grafana-cli ./bin/
